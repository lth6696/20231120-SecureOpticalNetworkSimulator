...
class CF:
    """
    Security First Service Provision (Capacity First Scheme) 算法实现
    ...
    def route(
        ...
        logging.debug(f"Call request {call.security} security.")
        # 步骤1: 搜索所有简单路径
        try:
        ...
        for path in k_paths[:k]:
            ...
            for (u, v) in zip(path[:-1], path[1:]):
                div_value += np.abs(
                    G[u][v]['link_security'] - np.ceil(call.security / 2)
                )
            div_value = div_value / (len(path) - 1)
            # print(div_value)
            ...
            # 综合评分
            evaluation = div_value * utl_value
            logging.debug(f"Evaluation of path {path} is {evaluation}.")

            available_paths.append((evaluation, path))

        # 检查可用路径集
        if available_paths:
            available_paths.sort(key=lambda x: x[0])    # 升序排列
        ...
        if call.security == 0 or call.security == 1:
            # 业务存在明文传输需求、尽力而为传输需求 --》 选择最小评估值的路径
            eval, path = available_paths.pop(0)
            # 允许普通业务使用安全带宽，但不使用加密能力
            logging.debug(f"Path {path} is conditional selected with the evaluation {eval}.")
            self._reserve_bandwidth(G, path, call)
            return True
        elif call.security == 2:
            # 业务存在安全需求，保证100%加密
            eval, path = available_paths.pop(0)
            if eval == 0:
                logging.debug(f"Path {path} is conditional selected with the evaluation {eval}.")
